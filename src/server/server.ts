...
const apiRoutes: Record<string, RouteHandler> = {
    ...
    'GET /api/assets/:id/image': async (_req, res, params, query) => {
        try {
            ...
            const formatMap: Record<string, string> = { jpg: 'jpeg', png: 'png', webp: 'webp' };
            ...
            if (requestedFormat && RASTER_FORMATS.includes(requestedFormat) && sharp) {
                try {
                    const qualityOpts: Record<string, Record<string, unknown>> = {
                    ...
                    let pipeline = isSvg
                        ...
                        : sharp(buffer);
                    const converted = await pipeline.toFormat(fmt as any, qualityOpts[fmt] || {}).toBuffer();
